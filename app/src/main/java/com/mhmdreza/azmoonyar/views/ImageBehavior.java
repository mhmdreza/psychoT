...
public class ImageBehavior extends CoordinatorLayout.Behavior<ImageView> {
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, ImageView child, View dependency) {
        ...
        float distanceXToSubtract = ((mStartXPosition - mFinalXPosition)
                * (1f - 0.7f)) + (child.getWidth()/2);

        float heightToSubtract = ((mStartHeight - finalHeight) * (1f - expandedPercentageFactor));

        child.setY(mStartYPosition - distanceYToSubtract);
        child.setX(mContext.getResources().getDisplayMetrics().widthPixels / 2 - child.getWidth() / 2);
        Log.e("TAG", "mStartYPosition: " + mStartYPosition + " mStartXPosition: " + mStartXPosition);
        Log.e("TAG", "distanceYToSubtract: " + distanceYToSubtract + " distanceXToSubtract: " + distanceXToSubtract);

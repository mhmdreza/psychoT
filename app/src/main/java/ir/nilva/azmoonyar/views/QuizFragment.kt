...
import ir.nilva.azmoonyar.data.quizes.FATHER
import ir.nilva.azmoonyar.data.quizes.TRADEOFF
import ir.nilva.azmoonyar.util.normalizeNumber
import kotlinx.android.synthetic.main.agreement_level_4.*
import kotlinx.android.synthetic.main.agreement_level_4.AL4four
import kotlinx.android.synthetic.main.agreement_level_4.AL4one
import kotlinx.android.synthetic.main.agreement_level_4.AL4three
import kotlinx.android.synthetic.main.agreement_level_4.AL4two
import kotlinx.android.synthetic.main.agreement_level_4.al4QuestionTitle
import kotlinx.android.synthetic.main.agreement_level_4.whichParent
...
class QuizFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        previousQuestion.setOnClickListener {
            if (questionNum <= 0) {
                Toast.makeText(view.context, "شما به ابتدای لیست سوالات رسیدید.", Toast.LENGTH_LONG)
                    .show()
        ...
        when (quiz.type) {
            ...
            AnswerType.AGREEMENT_LEVEL_4 -> {
                if (quiz.id == 7) {
                    firstLabel.text = "کاملا نادرست"
                    secondLabel.text = "کمی درست"
                    thirdLabel.text = "تقریبا درست"
                    fourthLabel.text = "کاملا درست"
                }
    ...
    private fun addNewQuizResult(answer: Int, questionSolved: Boolean, extra: String = "") {
        if (questionSolved) {
            quizResult.answerList.add(questionNum,
                Answer(answer, extra)
            )
            quizResult.answerList.removeAt(questionNum + 1)
        } else {
            quizResult.answerList.add(
                Answer(
                    answer,
                    extra
                )
    ...
    private fun updateProgressBar() {
        val animation = ObjectAnimator.ofFloat(
            progressBar,
            "progress",
            (questionNum * 100).toFloat(),
            ((questionNum + 1) * 100).toFloat()
        )
